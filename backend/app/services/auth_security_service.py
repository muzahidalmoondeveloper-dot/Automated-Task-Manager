...
class AuthSecurityService:
    ...
    async def check_ip_lock(self, ip_address: str) -> None:
        ...
        if locked_until > now:
            raise HTTPException(
                ...
                detail="Too many failed attempts. Your IP is locked for 3 minutes.",
            )
    ...
    async def verify_otp(
        ...
        *,
        ...
        stored_otp = normalize_otp(otp.otp_code)
    ...
    def login_otp_required(self, user: User) -> bool:
        ...
        return verified_at < datetime.now(timezone.utc) - timedelta(
            days=LOGIN_OTP_VALID_DAYS
        )