...
from app.core.dependencies import get_current_user, require_admin_or_team_manager
...
async def update_value(
    value_id: int,
    payload: OrgValueUpdate,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_admin_or_team_manager),
):
    result = await db.execute(select(OrgValue).where(OrgValue.id == value_id))
...
async def list_objectives(
    ...
    result = await db.execute(
        select(Objective)
        .options(selectinload(Objective.owner))
...
async def list_objective_rocks(
    db: AsyncSession = Depends(get_db),
    _: User = Depends(get_current_user),
):
    result = await db.execute(
        select(Rock)
        .where(Rock.objective_id.isnot(None))
        .order_by(Rock.objective_id, Rock.created_at.desc())
    )
    return result.scalars().all()
...
async def create_objective(
    ...
    obj = Objective(**payload.model_dump(), created_by_id=current_user.id)
    db.add(obj)
    await db.commit()
    result = await db.execute(
        select(Objective)
...
async def delete_objective(
    ...
    result = await db.execute(select(Objective).where(Objective.id == obj_id))
    obj = result.scalar_one_or_none()
    if not obj:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Objective not found.")
    await db.delete(obj)
...
async def list_roles(
    ...
    result = await db.execute(
        select(OrgRole)
        .options(selectinload(OrgRole.assignees))
        .order_by(OrgRole.created_at.asc())
    )
    return result.scalars().all()
...
async def create_role(
    ...
    result = await db.execute(
        select(OrgRole).where(OrgRole.id == role.id).options(selectinload(OrgRole.assignees))
    )
    return result.scalar_one()
...
async def update_role(
    ...
    result = await db.execute(
        select(OrgRole).where(OrgRole.id == role_id).options(selectinload(OrgRole.assignees))
    )
